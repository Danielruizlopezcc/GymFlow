...
import { COLORS, BODY_PART_COLORS, EQUIPMENT_ICONS, BODY_PART_ES, EQUIPMENT_ES } from '@/src/constants/theme';
import { getExerciseById, getSimilarExercises, type Exercise } from '@/src/data/exercises';
...
export default function ExerciseDetailScreen() {
  ...
  // Datos locales — sin red
  const exercise = useMemo(() => getExerciseById(id ?? ''), [id]);
  const similar  = useMemo(
    () => id ? getSimilarExercises(id, 6) : [],
    [id]
  );
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]} testID="exercise-detail-screen">
      ...
      <View style={[styles.heroSection, { backgroundColor: bodyColor + '10' }]}>
        <Image
          source={{ uri: getGifUrl(exercise.exercise_id) }}
        ...
        <View style={styles.heroMeta}>
          ...
          <View style={styles.heroBadge}>
            <Ionicons
              name={(EQUIPMENT_ICONS[exercise.equipment] || 'ellipsis-horizontal-outline') as any}
              size={14}
              color={COLORS.textSecondary}
            />
            <Text style={styles.heroBadgeText}>
              {EQUIPMENT_ES[exercise.equipment] || exercise.equipment}
            </Text>
          </View>
        </View>
      </View>
            ...
            {similar.length > 0 ? (
              similar.map(item => {
                ...
                return (
                  ...
                  >
                    <Image source={{ uri: getGifUrl(item.exercise_id) }} style={styles.similarGif} />
                    <View style={styles.similarInfo}>
                      <Text style={styles.similarName}>{item.name}</Text>
                      <View style={styles.similarMeta}>
                        <View style={[styles.similarBadge, { backgroundColor: simColor + '15' }]}>
                          <Text style={[styles.similarBadgeText, { color: simColor }]}>
...
const styles = StyleSheet.create({
  ...
  heroSection:           { alignItems: 'center', paddingVertical: 20, borderBottomWidth: 1, borderBottomColor: COLORS.border },
  heroGif:               { width: '100%', height: 200, marginBottom: 12 },
  heroMeta:              { flexDirection: 'row', gap: 10 },
  heroBadge:             { flexDirection: 'row', alignItems: 'center', gap: 6, paddingHorizontal: 14, paddingVertical: 6, borderRadius: 20, backgroundColor: COLORS.surface },
  heroBadgeText:         { fontSize: 13, fontWeight: '600', color: COLORS.textSecondary, textTransform: 'capitalize' },
  tabContainer:          { flexDirection: 'row', backgroundColor: COLORS.surface, marginHorizontal: 20, marginTop: 16, borderRadius: 12, padding: 4 },
  ...
  muscleCard:            { backgroundColor: COLORS.cardBg, borderRadius: 16, padding: 20, borderWidth: 1, borderColor: COLORS.border, marginBottom: 20 },
  muscleSection:         {},
  ...
  infoItem:              { flex: 1, alignItems: 'center', backgroundColor: COLORS.cardBg, borderRadius: 16, padding: 16, borderWidth: 1, borderColor: COLORS.border, gap: 8 },
  ...
  stepNumber:            { width: 32, height: 32, borderRadius: 10, backgroundColor: COLORS.accent, alignItems: 'center', justifyContent: 'center' },
  ...
  similarCard:           { flexDirection: 'row', alignItems: 'center', backgroundColor: COLORS.cardBg, borderRadius: 14, padding: 14, marginBottom: 10, borderWidth: 1, borderColor: COLORS.border, gap: 12 },
  similarGif:            { width: 48, height: 48, borderRadius: 12, backgroundColor: COLORS.surface },
  similarInfo:           { flex: 1 },
